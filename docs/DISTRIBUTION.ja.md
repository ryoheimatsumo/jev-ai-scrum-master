...
Skills CLIでプロジェクト単位に導入した場合は、そのプロジェクトのGitルートで実行します。
...
マーケットプレイス版はホストのプラグイン更新機能を使います。更新後はホストを再読み込みし、
更新版`jev-scrum-master`のランタイム準備を依頼してください。同梱wheelが変わると別のPython環境が
必要で、セットアップへの同意を改めて確認します。Skill削除は、実行環境、証拠、タスク履歴、
別に保管した認証情報の削除ではありません。
